...
export class AppWeatherVis implements ComponentInterface {
  ...
  private readonly variableOptions: string[] = ['Elevation', 'MaxTemperature', 'MinTemperature', 'Precipitation', 'Wind', 'RelativeHumidity', 'Solar'];
  private readonly variableNameAndColorSchemeDict = {
    Precipitation: ['rgb(84,48,4)', 'rgb(223,194,125)', 'rgb(109,234,229)', 'rgb(0,102,93)'],
  };
  ...
  private get colorScheme() {
    return this.variableNameAndColorSchemeDict[this.selectedVariables?.[0]] || ['#4575b4', '#abd9e9', '#fee090', '#f46d43'];
  }
    ...
    if (this.secondaryVisVariableNameAndCategorizedValuesDict) {
      ...
      for (let i = 0; i < secondaryVisCategorizedValuesForFirstDimension.length; i++) {
        if (i === 0 && secondaryVisCategorizedValuesForFirstDimension[0] !== firstVisCategorizedValuesForFirstDimension[0]) {
          ...
          i === secondaryVisCategorizedValuesForFirstDimension.length - 1 &&
          secondaryVisCategorizedValuesForFirstDimension[secondaryVisCategorizedValuesForFirstDimension.length - 1] !==
            firstVisCategorizedValuesForFirstDimension[firstVisCategorizedValuesForFirstDimension.length - 1]
        ) {
  ...
  private get secondaryVisDefineTexturesHandler() {
    if (this.secondaryVisVariableNameAndCategorizedValuesDict) {
      switch (this.textureType) {
          ...
          return (textureGenerator: any) => {
            ...
            for (let i = 0; i < secondaryVisCategorizedValuesForSecondDimension.length; i++) {
              if (i === 0 && secondaryVisCategorizedValuesForSecondDimension[0] !== firstVisCategorizedValuesForSecondDimension[0]) {
                ...
                i === secondaryVisCategorizedValuesForSecondDimension.length - 1 &&
                secondaryVisCategorizedValuesForSecondDimension[secondaryVisCategorizedValuesForSecondDimension.length - 1] !==
                  firstVisCategorizedValuesForSecondDimension[firstVisCategorizedValuesForSecondDimension.length - 1]
              ) {
        ...
        default:
          return textureGenerator => [1, 2, 3, 4, 5, 6].map(() => () => textureGenerator.circles().fill('transparent'));
      }
    } else {
  ...
  @State() visFillOpacity: number = 0.5;
  @State() visFillDimOpacity: number = 0.2;
  @State() visFillHighlightOpacity: number = 0.8;
      ...
      <Host>
        ...
        <ion-split-pane when="(min-width: 1600px)" content-id="main-container">
          <ion-menu side="end" content-id="main-container">
            <ion-list>
              <ion-item class="control-panel-item">
                ...
                <ion-range min={0} max={1} step={0.01} value={this.visFillOpacity} onIonChange={({ detail }) => (this.visFillOpacity = +detail.value)}>
              ...
              <ion-item class="control-panel-item">
                ...
                <ion-range min={0} max={1} step={0.01} value={this.visFillHighlightOpacity} onIonChange={({ detail }) => (this.visFillHighlightOpacity = +detail.value)}>
                  <ion-label slot="start">0</ion-label>
                  <ion-label slot="end">1</ion-label>
                </ion-range>
              </ion-item>
              <ion-item class="control-panel-item">
                ...
                <ion-range min={0} max={1} step={0.01} value={this.visFillDimOpacity} onIonChange={({ detail }) => (this.visFillDimOpacity = +detail.value)}>
                  <ion-label slot="start">0</ion-label>
                  <ion-label slot="end">1</ion-label>
                </ion-range>
              </ion-item>
              <ion-item class="control-panel-item">
                <ion-label class="control-panel-item-label">Texture Type</ion-label>
              ...
              <ion-item class="control-panel-item" disabled={!this.datasetInfo}>
                ...
                <ion-select multiple value={this.selectedVariables} onIonChange={async ({ detail }) => (this.selectedVariables = detail.value)}>
                  {this.variableOptions.map(variable => (
                    <ion-select-option>{variable}</ion-select-option>
                  ))}
                </ion-select>
              </ion-item>
              <ion-item class="control-panel-item" disabled={!this.datasetInfo}>
          ...
          <ion-content id="main-container" class="ion-padding">
            {this.data && this.selectedVariables?.length && this.renderMainVis()}
            {this.data && this.selectedVariables?.length && this.isSecondaryVisEnabled && this.renderMainVis(true)}
            {!this.file && (
              <ion-text>
                ...
                Please open a database file to start. If you do not have a database file, you will need to generate one. You can check the buttons on the upper-right corner to open
                a database file (Open Database File) or generate a new one (Generate Database File).
              </ion-text>
            )}
            {this.file && !this.selectedVariables?.length && (
              <ion-text>
                Please select some variables to display the visualization. You can configure parameters of the visualization from the side menu. (the suggested number of variables
                to be selected is from 2 to 4)
              </ion-text>
            )}
          </ion-content>
        </ion-split-pane>
    ...
    return (
      <ion-card class="vis-container">
        <s-set-stat
          ...
          data={isSecondaryVis ? this.secondaryVisData : this.data}
          parallel-sets-ribbon-tension={0.5}
          ...
          colorScheme={isSecondaryVis ? this.secondaryVisColorScheme : this.colorScheme}
          defineTexturesHandler={isSecondaryVis ? this.secondaryVisDefineTexturesHandler : this.defineTexturesHandler}
        ...
        ></s-set-stat>
        <app-map-view
          centerPoint={[(this.datasetInfo.maxLatitude + this.datasetInfo.minLatitude) / 2, (this.datasetInfo.maxLongitude + this.datasetInfo.minLongitude) / 2]}
          datasetRange={this.datasetInfo}
          zoom={5.5}
          header={isSecondaryVis ? this.secondaryVisMapViewHeader : this.mapViewHeader}
          heatmapData={isSecondaryVis ? this.secondaryVisMapViewHeatmapData : this.mapViewHeatmapData}
          heatmapOpacity={this.visFillOpacity}
          heatmapHighlightOpacity={this.visFillHighlightOpacity}
          onMouseDraw={isSecondaryVis ? ({ detail }) => (this.secondaryVisMapRange = detail) : ({ detail }) => (this.mapRange = detail)}
        ></app-map-view>
      </ion-card>
    );
  }
  ...
  private async statisticsColumnsHeaderClickHanlder(dimensionName: string, isSecondaryVis?: boolean) {
    ...
    if (isSecondaryVis) {
      await this.secondaryVisSetStatElement.reorderParallelSetsLastAxisByDimension(
        this.secondaryVisParallelSetsDateAxisSortedBy?.dimensionName,
        this.secondaryVisParallelSetsDateAxisSortedBy?.orderBy,
      );
    } else {
  ...
  private drawHeatmapOnMapView(dimensionName: string, dimensionValue: string | number, dataNodes: ParallelSetsDataNode[], isSecondaryVis?: boolean) {
  ...
  private async queryData(isForSecondaryVis?: boolean) {
    if (this.DB && this.selectedVariables?.length > 0) {
      ...
      let sqlQueryString = `select Date, Latitude, Longitude, ${this.selectedVariables.join(this.variableValueMinMaxSeperator)} ` + `from ${this.databaseName}`;
    ...
    if (data) {
      if (isForSecondaryVis) {
        ...
        for (const variableName of this.selectedVariables) {
          ...
          const firstVisCategorizedValues = this.variableNameAndCategorizedValuesDict?.[variableName]?.sort(
            (a, b) => +a.toString().split(this.variableValueMinMaxSeperator)[0] - +b.toString().split(this.variableValueMinMaxSeperator)[0],
          );
          ...
          if (variableMinValue < firstVisVariableMinValue) {
            this.secondaryVisVariableNameAndCategorizedValuesDict[variableName] = [
              `${variableMinValue}${this.variableValueMinMaxSeperator}${firstVisVariableMinValue}`,
              ...this.secondaryVisVariableNameAndCategorizedValuesDict[variableName],
            ];
          }
          if (variableMaxValue > firstVisVariableMaxValue) {
            this.secondaryVisVariableNameAndCategorizedValuesDict[variableName] = [
              ...this.secondaryVisVariableNameAndCategorizedValuesDict[variableName],
              `${firstVisVariableMaxValue}${this.variableValueMinMaxSeperator}${variableMaxValue}`,
            ];
          }
          for (const dataRecord of data) {
            ...
            dataRecord[`_${variableName}`] = this.secondaryVisVariableNameAndCategorizedValuesDict?.[variableName]
              ?.find((valueRange, index) => {
                ...
                return (
                  (variableValue >= +minValue && variableValue < +maxValue) ||
                  (index === this.secondaryVisVariableNameAndCategorizedValuesDict[variableName].length - 1 && variableValue === +maxValue)
                );
              })
        ...
        switch (this.categorizationMethod) {
          case 'quantile':
            ...
            this.selectedVariables.forEach(
              variableName =>
                (variableNameAndQuantileScaleDict[variableName] = d3
                  .scaleQuantile()
                  .domain(data.map(d => d[variableName]))
                  .range([0, 1, 2, 3])),
            );
          ...
          case 'uniform':
            ...
            this.selectedVariables.forEach(variableName => {
              this.variableNameAndCategorizedValuesDict[variableName] = this.variableNameAndCategorizedValuesDict[variableName].filter(
                v =>
                  data.filter(
                    d =>
                      d[`_${variableName}`] ===
  ...
  private obtainDatasetInfo(DB: SqlJs.Database) {
    try {
      const sqlQueryString =
        'select min(Latitude) as minLatitude, max(Latitude) as maxLatitude, count(distinct Latitude) as latitudeCount, min(Longitude) as minLongitude, max(Longitude) as maxLongitude, count(distinct Longitude) as longitudeCount from weather';
